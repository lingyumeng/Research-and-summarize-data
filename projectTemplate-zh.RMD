TITLE by YOUR_NAME_HERE
========================================================

```{r echo=FALSE, message=FALSE, warning=FALSE, packages}
# 加载你最终使用的所有组件
# 在这个代码块的分析中。

# 注意，在这个代码块中，将参数 "echo" 设为假。
# This prevents the code from displaying in the knitted HTML output.这可以避免代码混入 HTML 输出显示。
# 应当在文件中，对所有代码块设为 echo=FALSE 。

library(ggplot2)
```

```{r echo=FALSE, Load_the_Data}
# 加载数据
WhiteWine <- read.csv('wineQualityWhites.csv')
```
本报告探索了一个包含大约4900种白葡萄酒及其成分的数据集。


```{r echo=FALSE, Statistical_data_volume}
# 数据量统计
dim(WhiteWine)
```

```{r echo=FALSE, data_column_description}
# 查看数据集列信息
str(WhiteWine)
```
```{r echo=FALSE, Statistics_for_each_column}
# 分别对各列进行统计
summary(WhiteWine)
```
# 单变量绘图选择
```{r echo=FALSE, Univariate_Plots}
# 绘制品质直方图
ggplot(aes(x = quality), data = WhiteWine) +
  geom_histogram()
```
```{r echo=FALSE, map_fixed.acidity_histogram}
# 绘制固定酸度直方图
ggplot(aes(x=fixed.acidity), data = WhiteWine) +
  geom_histogram(binwidth = 0.1) +
  scale_x_continuous(limits = c(4,10))
```
```{r echo=FALSE, map_fixed.acidity_histogram}
# 绘制固定酸度直方图
ggplot(aes(x=fixed.acidity), data = WhiteWine) +
  geom_histogram(binwidth = 0.2) +
  scale_x_continuous(limits = c(4,10))
```

```{r echo=FALSE, map_volatile.acidity_histogram}
# 绘制挥发性酸度直方图
ggplot(aes(x=volatile.acidity), data = WhiteWine) +
  geom_histogram(binwidth = 0.02) +
  scale_x_continuous(limits = c(0.10, 0.75), breaks = seq(0,0.75, 0.05))
```

```{r echo=FALSE, map_citric.acid_histogram}
# 绘制柠檬酸直方图
ggplot(aes(x=citric.acid), data = WhiteWine) +
  geom_histogram(binwidth = 0.02) +
  scale_x_continuous(limits = c(0, 0.75), breaks = seq(0,0.75,0.05))
```
```{r echo=FALSE, map_residual.sugar_histogram}
# 绘制残留糖分直方图
ggplot(aes(x=residual.sugar), data = WhiteWine) +
  geom_histogram(binwidth = 0.5) +
  scale_x_continuous(limits = c(0,25))
```
```{r echo=FALSE map_chlorides_histogram}
# 绘制氯化物直方图
ggplot(aes(x=chlorides), data = WhiteWine) +
  geom_histogram(binwidth = 0.002) +
  scale_x_continuous(limits = c(0, 0.1), breaks = seq(0,0.1, 0.01))
```
```{r echo=FALSE map_free.sulfur.dioxide_histogram}
# 绘制游离二氧化硫直方图
ggplot(aes(x=free.sulfur.dioxide), data = WhiteWine) +
  geom_histogram(binwidth = 3) +
  scale_x_continuous(limits = c(0,90))
```
```{r echo=FALSE map_total.sulfur.dioxide_histogram}
# 绘制总二氧化硫直方图
ggplot(aes(x=total.sulfur.dioxide), data = WhiteWine) +
  geom_histogram(binwidth = 5) +
  scale_x_continuous(limits = c(0,275), breaks = seq(0, 275, 25))
```

```{r echo=FALSE, map_density_histogram}
ggplot(aes(x=density), data = WhiteWine) +
  geom_histogram(binwidth = 0.0005) +
  scale_x_continuous(limits = c(0.985, 1.01))
```
```{r echo=FALSE, map_pH_hist}
# 绘制ph值的直方图
ggplot(aes(x=pH), data = WhiteWine) +
  geom_histogram(binwidth = 0.03)
```
```{r echo=FALSE, map_sulphates_histogram}
# 绘制硫酸盐直方图
ggplot(aes(x=sulphates), data = WhiteWine) +
  geom_histogram(binwidth = 0.03)
```
```{r echo=FALSE, map_alcohol_histogram}
# 绘制乙醇直方图
ggplot(aes(x=alcohol), data = WhiteWine) +
  geom_histogram(binwidth = 0.2) +
  scale_x_continuous(limits = c(8, 15))
```


# 单变量分析

### 你的数据集结构是什么？

### 你的数据集内感兴趣的主要特性有哪些？

### 你认为数据集内哪些其他特征可以帮助你探索兴趣特点？

### 根据数据集内已有变量，你是否创建了任何新变量？

### 在已经探究的特性中，是否存在任何异常分布？你是否对数据进行一些操作，如清洁、调整或改变数据的形式？如果是，你为什么会这样做？



# 双变量绘图选择
```{r echo=FALSE, Bivariate_Plots}
ggplot(aes(x=fixed.acidity, y=quality), data = WhiteWine) +
  geom_point()
```

# 双变量分析

### 探讨你在这部分探究中观察到的一些关系。这些感兴趣的特性与数据集内其他特性有什么区别？

### 你是否观察到主要特性与其他特性之间的有趣关系？

### 你发现最强的关系是什么？




# 多变量绘图选择

```{r echo=FALSE, Multivariate_Plots}

```

# 多变量分析

###  探讨你在这部分探究中观察到的一些关系。通过观察感兴趣的特性，是否存在相互促进的特性？

### 这些特性之间是否存在有趣或惊人的联系呢？

### 选项：你是否创建过数据集的任何模型？讨论你模型的优缺点。

------

# 定稿图与总结

### 绘图一
```{r echo=FALSE, Plot_One}

```

### 描述一


### 绘图二
```{r echo=FALSE, Plot_Two}

```

### 描述二


### 绘图三
```{r echo=FALSE, Plot_Three}

```

### 描述三

------

# 反思